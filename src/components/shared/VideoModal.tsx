...
const VideoModal = () => {
  ...
  const { modalRef, contentRef, closeModal, videoUrl } = videoModal;
  ...
  return (
    <dialog
      ref={modalRef}
      className="modal-overlay modal-close fixed top-0 left-0 z-9999! size-full bg-black/95"
    ...
    >
      ...
      >
        {embedUrl ? (
          <iframe
            key={embedUrl}
            className="size-full rounded-[20px] max-[400px]:rounded-2xl"
            src={embedUrl}
            title="Video modal"
