...
export const useModal = (config: UseModalConfig = {}): UseModalReturn => {
  ...
  // Animation functions run only from openModal/closeModal (event/effect), not during render
  // eslint-disable-next-line react-hooks/refs -- refs read inside animation callbacks, not in render
  ...
  // Sync body scroll lock with modal open state (effect avoids "value cannot be modified" in callbacks)
  useEffect(() => {
